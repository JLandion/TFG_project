...
nombre_vars_hogares <- c(
  ...
  ingreso="D29",n_dormitorios="C22",m2="C23",ruido="C24_1",olores="C24_2",
...
lista_reemplazo_A <- list(
  list(cols = c("enf_cron","SM_estres","SM_satis","SM_frec_concentr","SM_frec_preoc", "SM_frec_desemp", 
                ...
                "al_frec_fruta","al_frec_verdura","al_frec_carne","al_frec_pescado",
                "al_frec_pasta","al_frec_legumbre","al_frec_lacteo"
              ),na_vals = c(8,9)),
...
lista_reemplazo_H <- list(
  list(cols = c("ruido","olores","agua","limpieza", "cont_indus","cont_otras",
...
# * Agrupamos los valores de actividad fisica 
datos_adultos$frec_act_fis[frec_act_fis == 1 | frec_act_fis == 2] <- 1 # No hace ejercicio
datos_adultos$frec_act_fis[frec_act_fis == 3 | frec_act_fis == 4] <- 0 # Si hace ejercicio
# Comprobamos
table(datos_adultos$frec_act_fis)

# * Agrupamos la variable situacion laboral
datos_adultos$sit_lab[sit_lab == 1 | sit_lab == 2 | sit_lab == 3 | sit_lab == 4 | sit_lab == 6 | sit_lab == 7] <- 0 # Si trabaja
datos_adultos$sit_lab[sit_lab == 5] <- 1 # No trabaja

# * Agrupamos la variable fumar
datos_adultos$fuma[fuma == 1 | fuma == 2] <- 1 # Si fuma
datos_adultos$fuma[fuma == 3 | fuma == 4] <- 0 # No fuma

datos_adultos$IMC[IMC == 4] <- 1
datos_adultos$IMC[IMC == 2 | IMC == 3] <- 0

# Dicotomizacion de las variables de caracterizacion de la vivienda
seleccion <- c("ruido", "olores","agua","limpieza", "cont_indus", "cont_otras",
               "escasez_verde", "molest_animal", "delincuencia")
datos_hogares[, seleccion] <- datos_hogares[, seleccion] * (datos_hogares[, seleccion] == 1)
...
datos_adultos$GHQ_12 <- as.integer(datos_adultos$GHQ_12 > 6)
